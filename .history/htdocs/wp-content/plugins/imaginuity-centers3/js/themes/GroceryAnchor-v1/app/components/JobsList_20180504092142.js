    ...
    componentWillMount(){
        ...
        let jobs = this.props.jobsData.map((job, i) => {
            ...
            if(moment().diff(job.acf.end_date, 'days') <= 0 ){
                jobCount++;
                return <Col xs={12} md={6} className="job border-color-setter" key={i}>
                    <Col xs={12} sm={job.acf.featured_image ? 6 : 12}>
                      <div className="copy-container font-color-setter">
                        <h4>
                          <Link to={"/jobs/" + job.slug} key={job.id}>
                            {entities.decode(job.title.rendered)}
                          </Link>
                        </h4>
                        {job.acf.related_store && <Link className="store-link" key={job.acf.related_store.ID} to={"/stores/" + job.acf.related_store.post_name} storeID={job.acf.related_store.post_name}>
