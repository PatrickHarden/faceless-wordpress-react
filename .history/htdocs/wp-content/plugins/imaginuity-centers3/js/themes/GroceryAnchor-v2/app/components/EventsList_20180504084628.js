    ...
    componentWillMount(){
        ...
        let events = this.props.eventsData.map((event, i) => {
            ...
            if(moment().diff(event.acf.end_date, 'days') <= 0 ){
                eventCount++;
                return(
                    <Col xs={12} md={6} className="event filter-item border-color-setter" data-startMonth={moment(event.acf.start_date).format('M')} data-endMonth={moment(event.acf.end_date).format('M')} data-categories={'[' + (event.event_categories ? event.event_categories : 0) + ']'} key={i}>
        ...
        if(this.state.data){
            return(
                <div className="events">
                    ...
                    <Grid>
                        <Row>
                            {this.state.eventCount > 0 ? this.state.events : <h2>There are no events currently available.</h2>}
                        </Row>
                    </Grid>
                </div>
            );
