...
class Jobs extends Component{
    ...
    componentWillMount(){
        ...
        axios.all([
            ...
            .then(axios.spread(function(site, page, home, options, jobs){
                let queries = [];
                if(jobs.headers['x-wp-totalpages'] > 1){
                    let paginationCount = jobs.headers['x-wp-totalpages'];
                    let x = 1;
                    while(x <= paginationCount){
                        queries.push(axios.get(JobsData + '&page=' + x));
                        x++;
                    }
                    axios.all(queries)
                        .then((response) => {
                    ...
                    component.handleDataResponse(site, page, home, options, jobs.data);
                }
            }))
            .catch((err) => {
                console.log('Error');
    ...
    handleDataResponse(site, page, home, options, jobs){
        this.setState({
            data: true,
            siteData: site.data,
            homeData: home.data[0],
