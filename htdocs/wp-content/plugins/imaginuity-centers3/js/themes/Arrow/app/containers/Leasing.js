        ...
        axios.all([
            ...
            .then(axios.spread(function (site, options, home, page) {
                                ...
                                {data.images && (
                                    <Carousel id={'carousel-' + data.location_number} className="location-carousel">
                                        {data.images.map(function(data, index){
                                            return <Carousel.Item key={index}><img src={data.image} /></Carousel.Item>
                                        })}
                                    </Carousel>
                                )}
                            </Panel>
                ...
                component.setState({
                    ...
                    spaces: spaces,
                    tripleCTAs: component.buildTripleCTA(page.data[0].acf.triple_ctas),
                });
            }))
            .catch((err) => {
                console.log(err);
        ...
        let CTAs = ctaData.map((cta, i) => {
            return(
                <Col xs={12} sm={10} smOffset={1} md={4} mdOffset={0} className="cta" key={i}>
                    {cta.url_type !== 'None' ? (
                        <a href={cta.url_type === "Internal" ? '/' + cta.cta_url.post_name : cta.cta_url_external} target={cta.url_type === 'External' ? '_blank' : ''} >
                            ...
                            <Row className="specialty-leasing">
                                ...
                                <Col xs={12} sm={6}>
                                    {this.state.pageData.acf.specialty_leasing_url ?
                                        (<a href={this.state.pageData.acf.specialty_leasing_url} target={this.externalLinkChecker(this.state.pageData.acf.specialty_leasing_url) ? '_self' : '_blank'}>
