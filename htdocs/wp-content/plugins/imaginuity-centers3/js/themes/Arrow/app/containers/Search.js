...
class Search extends Component{
    constructor(props) {
        ...
        this.state = {
            ...
            jobCount: 0,
            pageData: '',
            searchString: this.props.location.query.s.toLowerCase(),
            metaDescription: ''
        }
    }
    ...
    componentWillReceiveProps(nextProps){
        ...
        if(nextProps.location.query.s !== this.state.searchString){
            this.setState({
                data: false,
                searchString: nextProps.location.query.s.toLowerCase(),
            }, function(){
                this.search();
            });
        }
            ...
            .then(axios.spread((options, pages, stores, events, sales, jobs) => {
                ...
                if(pages.data){
                    pageResults = pages.data.map(function (data) {
                        ...
                        if(data.template === 'template-information.php'){
                            let layoutTypeTop = data.acf.layout_type_top[0].acf_fc_layout;
                            let layoutTypeBottom = data.acf.layout_type_bottom[0].acf_fc_layout;
                            let content;

                            switch(layoutTypeTop){
                                case 'full_width':
                                    content = data.acf.layout_type_top[0].content_area;
                                ...
                                case '2_column_wide_right':
                                    content = data.acf.layout_type_top[0].left_column + data.acf.layout_type_top[0].right_column;
                                    content = content.toLowerCase();
                                    if(content.indexOf(component.state.searchString) > -1){
                                        excerpt = content;
                                    }
                                    break;
                                case '4_column':
                                    content = data.acf.layout_type_top[0].column_1 + data.acf.layout_type_top[0].column_2 + data.acf.layout_type_top[0].column_3 + data.acf.layout_type_top[0].column_4;
                                    content = content.toLowerCase();
                                    if(content.indexOf(component.state.searchString) > -1){
                                        excerpt = content;
                                    }
                            ...
                            switch(layoutTypeBottom){
                                case 'full_width':
                                    content = data.acf.layout_type_bottom[0].content_area;
                                    content = content.toLowerCase();
                                    if(content.indexOf(component.state.searchString) > -1){
                                        excerpt = content;
                                    }
                                ...
                                case '2_column_wide_left':
                                case '2_column_wide_right':
                                    content = data.acf.layout_type_bottom[0].left_column + data.acf.layout_type_bottom[0].right_column;
                                    content = content.toLowerCase();
                                    if(content.indexOf(component.state.searchString) > -1){
                                        excerpt = content;
                                    }
                                    break;
                                case '4_column':
                                    content = data.acf.layout_type_bottom[0].column_1 + data.acf.layout_type_bottom[0].column_2 + data.acf.layout_type_bottom[0].column_3 + data.acf.layout_type_bottom[0].column_4;
                                    content = content.toLowerCase();
                                    if(content.indexOf(component.state.searchString) > -1){
                                        excerpt = content;
                                    }
                        ...
                        else if(data.template === 'template-leasing.php'){
                            excerpt = data.acf.leasing_space_copy + data.acf.specialty_leasing_copy;
                        }
                        else{
                            let content;
                        ...
                        return (
                            <div className="search-entry page-result font-color-setter">
                                ...
                                <p><div dangerouslySetInnerHTML={{__html: excerpt}}/></p>
                            </div>
                        );
                    });
                }
