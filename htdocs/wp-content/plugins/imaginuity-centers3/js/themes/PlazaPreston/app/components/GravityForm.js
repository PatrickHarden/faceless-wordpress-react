...
class GravityForm extends Component {
                                ...
                                let fields = data.response.fields.map(function (field) {
                                    // Create input type based off gForm Web API response, as well as accompanying state variable to match
                                    {
                                        switch (field.type) {
                                            ...
                                            case 'html':
                                                return (
                                                    <div className={"col-xs-12 form-group" + ' type-' + field.type}><Col
                                                        xs={12} dangerouslySetInnerHTML={{__html: field.content}}></Col>
                                                    </div>
                                                );
                                            case 'time':
                                                return (
                                                    <div
                                                        className={"col-xs-12 col-sm-6 form-group" + (field.isRequired ? ' required' : '') + ' type-' + field.type}>
    ...
    handleSubmit(event) {
        ...
        this.state.fieldList.map(function (field) {
            ...
            entry.input_values['input_' + fieldSanitized] = component.state[field];
        });
                ...
                success: function (data) {
                    if (data.status <= 202) {
                        if (data.response.is_valid) {
                            ...
                            var messages = data.response.validation_messages;
    ...
    handleValidationError(errors) {
        ...
        for (let i in messages) {
            console.log(i);
            console.log('Validation Error: ' + messages[i]);
            $('#' + i).closest('.form-group').addClass('has-error');
            $('#error-message').append("<p>" + messages[i] + "</p>");
        }
            ...
            return (
                <Row className="gform">
                    <Col xs={12}>
                        ...
                        <form title={this.state.gformTitle} className="gform" id={'gform-' + this.props.gformID}
                            ...
                            <Row className="fields">
                                ...
                                <Col xs={12} className="submit-container">
                                    <button type="submit" className="plaza-button">{this.props.submitLabel ? this.props.submitLabel : 'Submit'}</button>
                                </Col>
                            </Row>
                            <Row>
                                <Col xs={12} className="confirmation hidden"><p className="bg-success"><span
                                    className="glyphicon glyphicon-ok" aria-hidden="true"></span> <span
                                    className="message"
                                    dangerouslySetInnerHTML={{__html: this.state.confirmation}}></span></p></Col>
                                <Col xs={12} className="error hidden"><p className="bg-danger"><span
                                    className="glyphicon glyphicon-remove" aria-hidden="true"></span> An error occurred
                                    with this form. Please review your form entry and try again. If this error persists,
                                    contact the site administrator to alert them of this problem.</p></Col>
                            </Row>
                        </form>
                    </Col>
                </Row>
