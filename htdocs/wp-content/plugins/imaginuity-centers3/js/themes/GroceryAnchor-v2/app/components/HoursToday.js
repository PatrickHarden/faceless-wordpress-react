...
class HoursToday extends Component {
    ...
    buildHours(hoursData){
        ...
        dateString = 'Todays Hours / ' + (todayClosed ? 'Closed' : todayStart + ' - ' + todayEnd);
        ...
        if(this.state.data){
            return(
                <div className="hours-today">
                    <h4>{this.state.optionsData.acf.enable_hours_may_vary ? this.state.optionsData.acf.custom_hours_message : this.state.hours}</h4>
                </div>
            );
        }
        return null;
