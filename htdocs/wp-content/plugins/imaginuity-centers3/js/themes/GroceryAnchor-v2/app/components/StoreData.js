...
const BlogInfo = SiteURL + '/wp-json/ic3/v1/this_site';
const Stores = SiteURL + '/wp-json/wp/v2/stores';
const PropertyOptions = SiteURL + '/wp-json/acf/v3/options/property_options';
const Jobs = SiteURL + '/wp-json/wp/v2/jobs?per_page=100';
const Sales = SiteURL + '/wp-json/wp/v2/sales?per_page=100';
const SingleGlobalStore = SiteURL + '/wp-json/ic3/v1/global_store/';
    ...
    componentWillMount() {
                ...
                $.get(Stores + '?slug=' + this.state.storeSlug, function (data) {
                        ...
                        .then(function (response) {
                            ...
                            jobs = jobData.map(function (job) {
                                ...
                                if (jobStoreID === storeID) {
                                    hasJobs.push(
                                        <h3>
                                            <Link key={jobStoreID} onClick="jQuery('.modal').modal('hide')"
                                                  to={'/jobs/' + jobSlug + '/'}>{entities.decode(jobTitle)}</Link>
                                        </h3>
                                    );
                                }
                            });
                    ...
                    axios.get(Sales)
                        .then((response) => {
                                ...
                                if (typeof sale.acf.related_store !== 'undefined' && moment().isSameOrBefore(sale.acf.end_date) || nodate) {
                                    ...
                                    if (saleStoreID === storeID) {
                                        hasSales.push(
                                            <div>
                                                <h3>
                                                    <Link key={saleStoreID} onClick="jQuery('.modal').modal('hide')"
                                                          to={'/sales/' + saleSlug + '/'}>{entities.decode(saleTitle)}</Link>
                                                </h3>
                                                <p>{excerpt}</p>
                                            </div>
                                        );
                            ...
                            if (data.acf.global_store_selector && typeof data.acf.global_store_selector.selected_posts[0] !== 'undefined') {
                                console.log('global store found');
        ...
        $.get(PropertyOptions, function (data) {
            this.setState({
                ...
                enableJobsCTA: data.acf.enable_retailer_jobs_cta,
                enableSalesCTA: data.acf.enable_retailer_sales_cta,
            });
        }.bind(this));
    }
                ...
                <section className="store-details">
                    ...
                    <Grid>
                        <Row>
                            <Col xs={8} xsOffset={2}>
                                {(this.state.URL) ?
                                    <p><a href={this.state.URL}
                                          target="_blank">{this.state.URL.replace(window.location.protocol + '//', '').replace('https://', '').replace('www.', '')}</a>
                                    </p> : null}
                                {(this.state.image) ?
                                    <img src={this.state.image} className="img-responsive center-block featured-image"
                                         alt="Featured Image"/> : null}
                                <div className="content" dangerouslySetInnerHTML={{__html: this.state.copy}}></div>
                            </Col>
                        </Row>

                        <Row className="double-cta">
                            {this.state.enableJobsCTA &&
                            <Col xs={12} sm={6}
                                 className="col background-color-setter-lighter border-color-setter-lighter"
                                 onClick={this.openJobs.bind(this)}>
                                <h2>Jobs</h2>
                            </Col>
                            }
                            {this.state.enableSalesCTA &&
                            <Col xs={12} sm={6}
                                 className="col background-color-setter-lighter border-color-setter-lighter"
                                 onClick={this.openSales.bind(this)}>
                                <h2>Sales & Offers</h2>
                            </Col>
                            }
                        </Row>
                        ...
                        <Row className="store-info">
                            {this.state.restaurantMenu ? ( // if there is a restaurant menu, print 4 columns, else print 3 columns
                                <div>
                                        ...
                                        {this.state.phoneNumber ? (
                                            <div>
                                                <a href={'tel:' + this.state.phoneNumber} className="font-color-setter">
                                                    <svg id="phone-icon">
                                                        <circle className="cls-1 border-color-setter" cx="82.5"
                                                                cy="83.06" r="79.8"/>
                                            ...
                                            <div className="font-color-setter">
                                                <svg id="phone-icon">
                                                    <circle className="cls-1 border-color-setter" cx="82.5" cy="83.06"
                                                            r="79.8"/>
                                                    <path className="cls-2 background-color-setter"
                                                          d="M63.18,74.78a108.56,108.56,0,0,0,27.91,27.65c3-3,5.88-5.74,8.6-8.6,2.34-2.46,4.83-2.79,7.52-.82,3.62,2.66,7.26,5.3,10.7,8.18,3.93,3.29,4.28,6.5,1.57,11-4.32,7.17-13.46,11.29-21.62,7.9a107.09,107.09,0,0,1-23.62-13.38c-11.74-8.92-21.38-20-27.59-33.64-3.07-6.73-4.49-13.82-.29-20.53A26.72,26.72,0,0,1,54,45c3.21-2.17,6.76-1.64,9.35,1.58,3,3.76,6.24,7.44,8.63,11.58.94,1.63.61,4.89-.46,6.59C69.36,68.25,66.28,71.17,63.18,74.78Z"/>
                                                </svg>
                                                ...
                                                <a href={this.state.printableDirectory} target="_blank">
                                                    <svg id="location-icon">
                                                        <circle className="cls-1 border-color-setter" cx="82.5"
                                                                cy="83.06" r="79.8"/>
                                                        <path
                                                            className="cls-2 border-color-setter background-color-setter"
                                    ...
                                    <Col xs={12} sm={4}>
                                        ...
                                        <p><a className="font-color-setter" href={this.state.restaurantMenu}
                                              target="_blank">Restaurant Menu</a></p>
                                    </Col>
                                </div>
                            ) : (
                                <div>
                            ...
                            <div className="hours-container">
                                <Hours
                                    optionsData={this.props.storeData.acf.custom_hours ? this.props.storeData : this.props.optionsData}
                                    isStore={true} flags={this.props.storeData.acf.flags}/>
                            </div>
                            ...
                            <SocialScene fb={this.state.facebook} tw={this.state.twitter}
                                         ig={this.state.instagram} pn={this.state.pinterest}/>
