...
var Mapplic = function(element) {
    ...
    function Sidebar() {
        ...
        this.init = function() {
            ...
            this.notfound = $('<p></p>').addClass('mapplic-not-found').text('Not Found').appendTo(listContainer);
    ...
    var addControls = function() {
        ...
        if (self.o.mousewheel) {
            $('.mapplic-layer', self.el).bind('mousewheel DOMMouseScroll', function(e, delta) {
                ...
                self.scale = normalizeScale(scale + scale * e.originalEvent.wheelDelta / 5);
...
class Map extends Component{
            ...
            .then(() => {
                $.ajax({
                    ...
                    success: function(data){
                        ...
                        var element = $('#mapplic-id' + component.state.optionsData.acf.map_id);
                        var mapData = element.data('mapdata');

                        element.on('mapready', function(e, self) {
                            // Controls the text resizing for the map markers
                            element.on('positionchanged', function(e, pos) {
                                if(self.scale < 1){
                                    $('.mapplic-pin').css('font-size', '4px');
                                }
                                if(self.scale > 1 && self.scale < 2){
                                    $('.mapplic-pin').css('font-size', '9px');
                                }
                                if(self.scale >= 3){
                                    $('.mapplic-pin').css('font-size', '11px');
                                }
                            });
                        });
                        ...
                        var instance = (new Mapplic(element)).init(mapData);
    ...
    componentDidMount(){
        if(this.props.landmarkID){
            browserHistory.push(browserHistory.getCurrentLocation().pathname + '?location=' + this.props.landmarkID);
        }
    }
    ...
    mapplic(params) {
        //var len = this.length;
        var map = $('#mapplic-id' + this.state.optionsData.acf.map_id);
        var data = map.data('mapdata');
        var element = map;

        var instance = (new Mapplic(element)).init(data);
    ...
    render(){
        if(this.state.data && this.state.optionsData.acf.enable_interactive_map){
            console.log(this.state.optionsData);
            return(
                <div>
                    <Link to={'/directory'} className="aria-only hidden">You are viewing the interactive map. To navigate to the directory for a list of stores, click this link</Link>
                    <div className="interactive-map" aria-hidden="true" role="presentation">
                        <span dangerouslySetInnerHTML={{ __html: this.state.map}} />
                    </div>
                </div>
            );
        }
