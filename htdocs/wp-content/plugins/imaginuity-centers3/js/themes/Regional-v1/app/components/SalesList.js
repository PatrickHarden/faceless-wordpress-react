    ...
    componentWillMount(){
        ...
        let sales = this.props.salesData.map((sale, i) => {
            ...
            if(moment().diff(sale.acf.end_date, 'days') <= 0 || nodate){
                ...
                return(
                    <div className="sale filter-item" data-startMonth={moment(sale.acf.start_date).format('M')}
                         data-endMonth={moment(sale.acf.end_date).format('M')}
                         data-store={entities.decode(sale.acf.related_store.post_title)}
                         startdate={moment(sale.acf.start_date).format('DDDD')}
                         enddate={moment(sale.acf.end_date).format('DDDD')}
                         key={i}>
                        {sale.acf.featured_image &&
                            <Link to={"/sales/"+sale.slug+"/"}>
                                <img src={entities.decode(sale.acf.featured_image)} alt={entities.decode(sale.title.rendered) + " featured image"} />
                            </Link>
                        }
                        {!nodate &&
                        ...
                        <div className="copy-container font-color-setter background-color-setter-lightest">
                            ...
                            {excerpt &&
                            <p>{excerpt}<Link to={"/sales/"+sale.slug+"/"} className="font-color-setter">Read More</Link></p>
                            }
                        </div>
                    </div>
                );
        ...
        let sortedSales = sales.sort(function(a, b) {
            if(a.props.enddate === "" || a.props.enddate === null) return 1;
            if(b.props.enddate === "" || b.props.enddate === null) return -1;
            if(a.props.enddate === b.props.enddate) return 0;
            return a.props.enddate < b.props.enddate ? -1 : 1;
        });
            ...
            if(this.state.saleCount > 0){
                return(
                    <div className="sales-data">
                        {this.props.pageData.acf.show_months_filter &&
                        <MonthFilter filter={this.setMonthFilter.bind(this)} className="hidden" />
                        }
                        <DropdownFilter
                            filterType="stores"
                ...
                return(
                    <div className="sales-data">
                        <Grid>
                            <Row>
                                <Col xs={12}>
                                    <h2>There are no sales currently available.</h2>
                                </Col>
                            </Row>
                        </Grid>
                    </div>
