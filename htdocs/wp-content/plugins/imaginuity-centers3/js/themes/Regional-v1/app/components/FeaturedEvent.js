...
class FeaturedEvent extends Component{
    ...
    render(){
        ...
        if(this.state.data){
            ...
            let rangeStartDate = (event.acf.start_date ? moment(event.acf.start_date).format('MM/DD') : '');
            let rangeEndDate = (event.acf.end_date ? moment(event.acf.end_date).format('MM/DD') : '');

            var startTime = (event.acf.start_time ? moment(event.acf.start_time, 'h:mm a').format('h:mm a') : '');
            var endTime = (event.acf.end_time ? moment(event.acf.end_time, 'h:mm a').format('h:mm a') : '');
            let timeString = startTime !== endTime ? (startTime == '' && endTime== '' ? 'All Day' : startTime + ' - ' + endTime) : startTime;
            timeString = startTime == '' && endTime == '' ? 'All Day' : timeString;
            ...
            return(
                <div className="featured-event-single background-color-setter">
                    <div className="img-container">
                        <img src={this.state.event.acf.featured_event_image.url} alt={(this.state.event.acf.featured_event_image.alt ? this.state.event.acf.featured_event_image.alt : 'featured image')}/>
                    </div>
                    <div className="copy-container">
                        ...
                        <p>{excerpt}<Link to={"/events/"+this.state.featuredEvent.slug+"/"}  alt={"navigate to interior page for " + entities.decode(this.state.event.acf.featured_event.post_title)}>Read More</Link></p>
                    </div>
